...
{
    // Create Window
    InitWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Pixel Perfect Collision");
    // Set Max FPS
    SetTargetFPS(60);
    ...
    Slider carScaleSldr = Slider(sldrRect, 5, 20, 5, GRAY, RAYWHITE, BLUE, RED, "Car Scale: ", 30);
    sldrRect = Rectangle{10, 185, 400, 75};
    Slider carRotationSldr = Slider(sldrRect, 0, 360, 0, GRAY, RAYWHITE, BLUE, RED, "Car Rotation: ", 30);

    // Slider for Some Object scale
    sldrRect = Rectangle{10, 270, 400, 75};
    Slider someObjectScaleSldr = Slider(sldrRect, 5, 20, 5, GRAY, RAYWHITE, BLUE, RED, "Rec. Scale: ", 30);
    ...
    while (!WindowShouldClose())
    {
        ...
        ClearBackground(Color{75, 75, 75, 255});
        {
            ...
            // Draw top panel
            if (GetSpritePixelCollision(&car, &someObject, debugDraw)) // Check collision between sprites and highlight the colliding pixels if enabled
            { // Sprites collided
                DrawRectangle(0, 0, SCREEN_WIDTH, 50, RED);
                DrawText("COLLISION!", SCREEN_WIDTH / 2 - MeasureText("COLLISION!", 40 / 2), 10, 40, RAYWHITE);
            }
            else
            { // Sprites didn't collide
